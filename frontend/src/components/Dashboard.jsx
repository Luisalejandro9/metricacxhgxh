...
function Dashboard({ user, profile, setNetworkError }) {
  ...
  return (
    <div className="app-layout">
      ...
      {/* HELP MODAL */}
      <div className={`modal-overlay ${showHelpModal ? 'active' : ''}`}>
        <div className="modal" style={{ maxWidth: '700px', maxHeight: '90vh', overflowY: 'auto' }}>
          <div className="modal-header">
            <h2>Manual de la Plataforma</h2>
            <button className="btn-close" onClick={() => setShowHelpModal(false)}><X /></button>
          </div>
          ...
          <div className="modal-footer" style={{ marginTop: '25px', display: 'flex', justifyContent: 'flex-end' }}>
            <button className="btn btn-primary" onClick={() => setShowHelpModal(false)}>¡Entendido!</button>
          </div>
        </div>
      </div>
    </div>
